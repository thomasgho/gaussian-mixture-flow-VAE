...
class ODEfunc(nn.Module):
    ...
    def __init__(self, dim, zero_initialization=True, batch_norm=True):
        super(ODEfunc, self).__init__()
        self.linear_layers = nn.ModuleList([nn.Linear(dim, dim)
                                            for _ in range(2)])
        self.relu = nn.ReLU()
        ...
        if batch_norm:
            self.batch_norm_layers = nn.ModuleList([nn.BatchNorm1d(dim,
                    eps=1e-3) for _ in range(2)])
...
class ODEnet(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([ODEblock(ODEfunc(hidden_dim), tol)
        ...
        if self.encode and self.gaussian_mix:
            self.final_layers_loc = nn.ModuleList(
                [nn.Linear(hidden_dim, out_dim)
                 for _ in range(self.num_gauss)])
            self.final_layers_scale = nn.ModuleList(
                [nn.Linear(hidden_dim, out_dim)
                 for _ in range(self.num_gauss)])
            self.final_layers_weight = nn.ModuleList(
    ...
    def forward(self, inputs):
        ...
        if self.encode and not self.gaussian_mix:
            ...
            mus = torch.stack([self.final_layers_loc[n](temps) for n in
                              range(self.num_gauss)])
            logvars = torch.stack([self.final_layers_scale[n](temps)
                                  for n in range(self.num_gauss)])
            weights = torch.stack([self.final_layers_weight[n](temps)
                                  for n in
                                  range(self.num_gauss)]).squeeze()
